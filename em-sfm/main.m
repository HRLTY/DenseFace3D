...
%load('../input_and_truth.mat');
input_2d = dlmread('/Users/huangrui/Developer/dlib-18.18/examples/3dMesh/xcode/Release/track.txt');
P3_gt = [input_2d;zeros(size(input_2d,1) / 2, size(input_2d,2))];
%P3_gt = groundtruth;
...
max_em_iter = 20;
tol = 0.0001;
K = 4; % number of deformation shapes
...
data = P3(2*T:end,:);
averageDepth = mean(data);
deviation = std(data);
averageDepth = averageDepth ./ deviation;
%minData=min(min(data));
%maxData=max(max(data));
%minmaxeddata = ((averageDepth-minData)./(maxData));
dlmwrite('/Users/huangrui/Developer/dlib-18.18/examples/3dMesh/xcode/Release/new_depth.txt',...
averageDepth, ' ');
